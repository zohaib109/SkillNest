				...
				<div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_23rem]">
					<section className="overflow-hidden rounded-3xl border border-border bg-card shadow-2xs">
						<div className="flex flex-col gap-5 border-b border-border p-5 sm:flex-row sm:p-7">
							...
							<div className="min-w-0">
								<p className="text-sm font-semibold text-primary">Verified SkillNest tutor</p>
						...
						<div className="grid gap-5 p-5 sm:p-7">
							<section>
								<h2 className="text-lg font-semibold text-foreground">About {profile.userName}</h2>
								<p className="mt-2 text-sm leading-7 text-muted-foreground">{bioPreview}</p>
								{remainingBio && (
									<details className="group mt-2">
										...
										<p className="mt-2 text-sm leading-7 text-muted-foreground">{remainingBio}</p>
									</details>
								)}
							</section>
							...
							{profile.availabilityRules.length > 0 && (
								<section>
									<h2 className="text-lg font-semibold text-foreground">Typical weekly hours</h2>
									<div className="mt-3 flex flex-wrap gap-2">
										{profile.availabilityRules.map((rule, index) => (
											<span
												key={`${rule.dayOfWeek}-${rule.startTime}-${index}`}
							...
							{embedUrl && (
								<section>
									<h2 className="mb-3 text-lg font-semibold text-foreground">Meet your tutor</h2>
					...
					<aside className="sticky top-20 rounded-3xl border border-border bg-card p-5 shadow-2xs">
						<div className="border-b border-border pb-4">
							<p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Starting rate</p>
						...
						{isStudent ? (
							...
							<p className="pt-4 text-sm leading-relaxed text-muted-foreground">
								This is your live profile. Students can send requests from this panel.
							</p>
						) : (
							<p className="pt-4 text-sm leading-relaxed text-muted-foreground">
